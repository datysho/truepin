...
<p>
  ...
  <img src="https://img.shields.io/badge/version-3.7.3-2563eb?style=flat-square" alt="version 3.7.3">
...
The popup (on the toolbar icon) shows the current pinned tabs with favicons and a lock on each protected one, saving the set under a name, the list of sets, and a collapsible list of autosaves. It also lists any manually-locked non-pinned tabs in a **LOCKED** section.
...
**Tests** - end-to-end on Puppeteer against a real Chrome for Testing, 28 scenarios (immortality under every close method, the reopen notification, free reload and navigation, the unpin-then-close path, manual lock carried over, mirroring across three windows, adoption without duplicates, the split-view rules, snapshot diff-restore, autosave triggers, localization, and a clean service worker log):
