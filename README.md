...
- The popup shows the current pinned tabs (with favicons), a 🔒 on each protected one, saving the set under a name, the list of sets and a collapsible list of autosaves. It also lists any manually-locked non-pinned tabs in a LOCKED section. The lock is drawn from each tab's real protected state, so it is correct even for a discarded pin.
